...
bool Utils::isPremiumNumber(const QString &text) const
{
    ...
    QString modified = text.right(text.length() - index);
    modified.replace(SL("-"), SL("")).replace(SL(" "), SL(""));

    return premiumRegex.match(modified).hasMatch();
}
